...
class _StaffInvitationScreenState extends State<StaffInvitationScreen> {
  ...
  String _selectedRole = constData.roleMember;
  ...
  static const Map<String, String> _invitableRoles = {
    constData.roleAdmin: '管理担当(admin)',
    constData.roleMember: '一般ユーザー(member)',
  };
  ...
  // ログインユーザーの企業情報をロード (仕様維持)
  Future<void> _loadMyCompanyInfo() async {
    try {
      final uid = FirebaseAuth.instance.currentUser?.uid;
      if (uid == null) {
        setState(() {
          _errorMessage = "ログイン情報が確認できません。";
          _isLoading = false;
        });
        return;
      }
      ...
      if (companyCode.toString().isEmpty) {
        setState(() {
          _errorMessage = "所属企業（法人コード）が設定されていないため、招待を送信できません。";
          _isLoading = false;
        });
        return;
      }
      ...
      String companyName = '';
      final companyDoc =
          await _db.collection('companies').doc(companyCode).get();
  ...
  Future<void> _sendInvitation() async {
    ...
    try {
      ...
      final roleLabel = UIUtils.getRoleDisplayName(_selectedRole);
      // ★ システム名を定数化
      final subject = '【重要】$_myCompanyName ${constData.systemName} アカウント登録のご案内';
      final body = '${_nameController.text} 様\n\n'
          'お世話になっております。$_myDisplayName です。\n'
          '$_myCompanyName の ${constData.systemName} アカウントを発行いたしました（権限: $roleLabel）。\n\n'
      ...
      final String gmailUrl =
          'https://mail.google.com/mail/?view=cm&fs=1&to=$toEmail&su=$encodedSubject&body=$encodedBody';
        ...
        title: Row(
          children: [
            const Icon(Icons.person_add_rounded,
                color: constData.themeGreen, size: 24),
        ...
        child: Center(
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  '$_myCompanyName（$_myCompanyCode）へのスタッフ招待',
        ...
        child: Column(
          ...
          children: [
            ...
            _isSending
                ? const Center(
                    child:
                        CircularProgressIndicator(color: constData.themeGreen))
              ...
              children: [
                ...
                TextButton.icon(
                  ...
                  style: TextButton.styleFrom(
                      foregroundColor: constData.themeGreen),
                ),
              ],
            ),
            const Divider(),
